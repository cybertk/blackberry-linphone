...
public class SipURI extends URI {
	...
	public SipURI() {
		...
		this.uriParms.setSeparator(Separators.SEMICOLON);
	...
	public void setScheme(String scheme) throws IllegalArgumentException {
		if (compareToIgnoreCase(scheme, SIP) != 0
				&& compareToIgnoreCase(scheme, SIPS) != 0)
	...
	public void clearUriParms() {
        ...
        // Little spelling fix by Arnau Vazquez (Genaker)
        uriParms.setSeparator(Separators.SEMICOLON);
    }
	...
	public boolean equals(Object that) {
		...
		for (int i = 0; i < uriParms.size(); i++) {
			NameValue nv = (NameValue) uriParms.elementAt(i);
			// transport string defaults to udp.
			if (nv.getName().equals(ParameterNames.TRANSPORT)) {
				String value = (String) nv.getValue();
				String hisTransport = (String) hisParms
						.getValue(ParameterNames.TRANSPORT);
				if (hisTransport == null
						&& compareToIgnoreCase(value, ParameterNames.UDP) == 0) {
					continue;
				} else if (hisTransport == null) {
					return false;
				} else if (compareToIgnoreCase(hisTransport, value) == 0) {
					continue;
				}
			} else {
	...
	public String encode() {
		StringBuffer retval = new StringBuffer(scheme).append(Separators.COLON);
		if (authority != null)
	...
	public String getUserAtHost() {
		...
		return new StringBuffer(user).append(Separators.AT).append(host)
				.toString();
	}
	...
	public String getUserAtHostPort() {
		...
		if (port != -1) {
			return s.append(host).append(Separators.COLON).append(port)
					.toString();
		} else
	...
	public String getMethod() {
		return (String) getParm(ParameterNames.METHOD);
	}
	...
	public void removeUriParms() {
		...
		// code added for encoding of multiple Parameters
		this.uriParms.setSeparator(Separators.SEMICOLON);
	...
	public String getUserType() {
		return (String) uriParms.getValue(ParameterNames.USER);
	}

	/**
	 * Get the password of the user.
	 * 
	 * @return User password when it embedded as part of the uri ( a very bad
	 *         idea).
	 */
	...
	/**
	 * Returns the stucture corresponding to the telephone number provided that
	 * the user is a telephone subscriber.
	 * 
	 * @return TelephoneNumber part of the url (only makes sense when user =
	 *         phone is specified)
	 */
	...
	/**
	 * returns true if the user is a telephone subscriber. If the host is an
	 * Internet telephony gateway, a telephone-subscriber field MAY be used
	 * instead of a user field. The telephone-subscriber field uses the notation
	 * of RFC 2806 [19]. Any characters of the un-escaped "telephone-subscriber"
	 * that are not either in the set "unreserved" or "user-unreserved" MUST be
	 * escaped. The set of characters not reserved in the RFC 2806 description
	 * of telephone-subscriber contains a number of characters in various syntax
	 * elements that need to be escaped when used in SIP URLs, for example
	 * quotation marks (%22), hash (%23), colon (%3a), at-sign (%40) and the
	 * "unwise" characters, i.e., punctuation of %5b and above.
	 * 
	 * The telephone number is a special case of a user name and cannot be
	 * distinguished by a BNF. Thus, a URL parameter, user, is added to
	 * distinguish telephone numbers from user names.
	 * 
	 * The user parameter value "phone" indicates that the user part contains a
	 * telephone number. Even without this parameter, recipients of SIP URLs MAY
	 * interpret the pre-@ part as a telephone number if local restrictions on
	 * the
	 * 
	...
	/**
	 * Sets the user of SipURI. The identifier of a particular resource at the
	 * host being addressed. The user and the user password including the "at"
	 ...
	 * already set to some value. Otherwise set it to the given value.
	 * 
	 * @param name
	...
	/**
	 * Set a given URI parameter. Note - parameter must be properly encoded
	 ...
	 * 
	 * @param userParam -
	 *            new value String value of the method parameter
	 */
	 ...
	 * 
	 * @param tel
	 *            Telephone subscriber field to set.
	 */
	 ...
	 * 
	 * @param qHeader -
	 *            qeuery header provided as a name,value pair.
	 */
	...
	public Object clone() {
		...
		if (this.telephoneSubscriber != null) {
			retval.telephoneSubscriber = (TelephoneNumber) this.telephoneSubscriber
					.clone();
		}
		return retval;
	}
	...
	/**
	 * Returns the value of the named header, or null if it is not set. SIP/SIPS
	 * URIs may specify headers. As an example, the URI
	 * sip:joe@jcp.org?priority=urgent has a header "priority" whose value is
	 * "urgent".
	 * 
	 * @param <var>name
	 *            </var> name of header to retrieve
	 * @return the value of specified header
	 */
	public String getHeader(String name) {
		return this.qheaders.getValue(name) != null ? this.qheaders.getValue(
				name).toString() : null;

	}

	/**
	 * Returns an Iterator over the names (Strings) of all headers present in
	 * this SipURI.
	 * 
	...
	/**
	 * Returns the value of the <code>lr</code> parameter, or null if this is
	 * not set. This is equivalent to getParameter("lr").
	 * 
	 ...
	 * 
	 * @param <var>name
	 *            </var> name of parameter to retrieve
	 * 
	...
	/**
	 * Returns the value of the "ttl" parameter, or -1 if this is not set. This
	 * method is equivalent to getParameter("ttl").
	 * 
	...
	/**
	 * Returns the value of the <code>userParam</code>, or null if this is
	 * not set.
	 * <p>
	...
	public boolean isSecure() {
		return equalsIgnoreCase(this.getScheme(), SIPS);
	}
	...
	/**
	 * Sets the value of the specified header fields to be included in a request
	 * constructed from the URI. If the header already had a value it will be
	 * overwritten.
	 * 
	 * @param name -
	 ...
	 * parameter, when present, indicates that the element responsible for this
	 * resource implements the routing mechanisms specified in RFC 3261. This
	 * parameter will be used in the URIs proxies place in the Record-Route
	 * header field values, and may appear in the URIs in a pre-existing route
	 * set.
	 */
	 ...
	 * 
	 * @param method -
	 *            new value of the <code>maddr</code> parameter
	 */
	 ...
	 * 
	 * @param secure -
	 *            the boolean value indicating if the SipURI is secure.
	 */
	 ...
	 * 
	 * @param ttl -
	 *            new value of the <code>ttl</code> parameter
	 */
	 ...
	 * setParameter("transport", transport). Transport parameter constants are
	 * defined in the {@link javax.sip.ListeningPoint}.
	 * 
	 * @param transport -
	...
	public void setTransportParam(String transport) throws ParseException {
		...
		if (equalsIgnoreCase(transport, "UDP")
				|| equalsIgnoreCase(transport, "TCP")) {
